...
```
[1] Idea + Script (Claude)  ──►  ✅ APPROVAL #1 (script)
        │
...
|---|---|---|---|
| Script + metadata | `scripts/generate_script.py` | Claude API | Kids story split into scenes; each scene has narration + an image prompt |
...
```bash
JOB=$(date +%s)
python scripts/generate_script.py    --job "$JOB" --topic "a shy little cloud learns to make rain"
